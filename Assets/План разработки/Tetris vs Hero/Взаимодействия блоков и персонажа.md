  ...
  └─ OnAttack(cx + dir, cy, dir)
       └─ BlockInteraction.HandleAttack
            └─ ячейки (x, cy) и (x, cy+1)
                 ├─ DamageCell(x, y)        ← HP-- для любого блока
                 │    └─ если HP = 0 → ячейка пуста → VictoryModel.AddDestroyedBlocks(1)
                 └─ PlayHitStrong()          ← толчок при каждом ударе
```

> [!tip] Все блоки имеют HP = 3
> 1-й и 2-й удар — толчок + затемнение. 3-й — ломает блок.
...
`BoardRenderer.RefreshCell` затемняет цвет пропорционально урону (для всех блоков):
...
[Персонаж] J → HandleAttack
  └─ OnAttack(cx-1, cy, -1)
       └─ HandleAttack
            ├─ DamageCell → HP--
            │    └─ HP = 0 → ячейка пуста → AddDestroyedBlocks → [если порог] OnCharacterWin
            └─ PlayHitStrong   ← толчок при каждом ударе
```
...
> - [x] HP-визуал для всех блоков (`CellData.MaxHP` + `BoardRenderer`)
> - [x] Толчок при каждом ударе (`PlayHitStrong` в `BlockInteraction`)
> - [x] Все блоки одинаковые: HP=3, 3 удара = ломается
